...
ggplot(beit.bridge,aes(x = Level..m., y = Flow..cumec.))+
  geom_point()
...
ggplot(mutale.clean,aes(RiverStage_m))+
  geom_density()
...
#Dunn's test - non-parametric equivalent to Tukey
dunnTest(RiverStage_m ~ Quarter,
         data=riverstage.monthly,
         method="bonferroni")

ggplot(riverstage.monthly,aes(x = Quarter, y = RiverStage_m))+
  geom_boxplot()
...
ggplot(beit.bridge.clean,aes(Year,Flow..cumec.))+
  geom_point()+
  geom_line()+
...
ggplot(data = drainage, aes(CatchmentArea_km2)) +
  geom_density()

ggplot(data = drainage, aes(log10(CatchmentArea_km2))) +
  geom_density()
...
write.csv(mutale.clean, file = 'mutale_clean.csv')
